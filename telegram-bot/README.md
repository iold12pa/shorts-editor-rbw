...
## ⚠️ Trạng thái thật: đã tự kiểm tra từng phần thật (không chỉ đọc code), CHƯA chạy thử đầu-cuối bằng bot Telegram thật

Xem chi tiết trong code `bot.py` (phần comment đầu file) — đã sửa 1 lỗi kiến trúc lớn (gói phần mềm ban đầu dùng không tồn tại, đã đổi sang gọi thẳng chương trình `claude` thật), đã tự chạy thật và bắt được thêm 3 lỗi: encoding Unicode khi đọc phản hồi trên Windows, cách lấy vòng lặp async không còn đúng với bản thư viện Telegram mới, và bug khiến mạng chập chờn báo lỗi giả — cả 3 đã sửa. Ai là người đầu tiên cài thật sẽ cần báo lại lỗi khác máy khác môi trường nếu có.
...
Làm theo hướng dẫn trên màn hình — trình duyệt sẽ mở ra xác nhận đăng nhập.

**Quan trọng**: lệnh này KHÔNG tự lưu đăng nhập vào máy — nó chỉ IN RA màn hình PowerShell 1 chuỗi ký tự dài (token), có hiệu lực 1 năm. Copy đúng chuỗi đó, dán vào `config.json` ở mục `claude_oauth_token` (xem Bước 2). Đừng lo nếu gõ `claude auth status` vẫn thấy báo "chưa đăng nhập" — bình thường, vì lệnh đó kiểm tra kiểu đăng nhập khác (Desktop), không liên quan tới token dùng cho bot này.
...
# Mở config.json, dán bot token (Bước 1) VÀ token từ setup-token (Bước 0) vào, lưu lại
python bot.py
```

Nếu báo "chua co 'claude_oauth_token' hop le" → quay lại Bước 0, dán đúng chuỗi token vào `config.json`. Nếu báo "Goi thu claude.exe that bai" → token có thể sai/hết hạn, chạy lại `claude setup-token` lấy token mới. Nếu báo "Khong tim thay claude.exe" → mở `config.json`, điền đường dẫn đầy đủ tới `claude.exe` vào `claude_binary_path`.
