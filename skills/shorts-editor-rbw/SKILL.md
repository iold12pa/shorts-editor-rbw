...
**Nhân tiện mỗi lần kiểm phiên bản: dọn cache bản cũ** — thư mục `~/.claude/plugins/cache/roboworld-tools/shorts-editor-rbw/` chứa mỗi bản đã cài 1 folder con (tên = mã bản); các bản CŨ (khác mã bản đang dùng theo `claude plugin list`) là rác chiếm chỗ vô ích (máy cài từ thời kho tài nguyên còn trong repo có thể đọng hàng trăm MB) — xóa hết folder bản cũ, GIỮ đúng folder bản đang dùng, báo 1 câu đã giải phóng bao nhiêu. **Chỉ xóa trong `plugins/cache/...` — KHÔNG BAO GIỜ đụng tới `~/.claude/roboworld-assets/` dưới bất kỳ hình thức nào** (chỗ đó chứa model Whisper, kho tài nguyên, và file `tuy-chinh-rieng.md` — xóa nhầm là mất tùy chỉnh + phải tải lại 1.6GB).
