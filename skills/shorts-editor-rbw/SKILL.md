...
**Tùy chỉnh riêng theo máy (không bị auto-update ghi đè):** trước khi áp dụng bất kỳ quy chuẩn nào ở các mục bên dưới (kịch bản, style, ffmpeg...), kiểm tra xem có file `references/tuy-chinh-rieng.md` không. File này KHÔNG nằm trong git (mỗi máy tự có bản riêng nếu người dùng máy đó từng yêu cầu tùy biến qua hội thoại) — nếu tồn tại, đọc và **ưu tiên áp dụng nội dung trong đó**, ghi đè lên phần quy chuẩn chung tương ứng bên dưới cho video đang làm. Nếu người dùng (ở BẤT KỲ máy nào, kể cả máy không có quyền push GitHub) yêu cầu đổi 1 quy tắc dựng nào đó ("từ giờ chữ hook màu khác", "đổi outro", "không dùng nhạc nền này nữa"...), LUÔN ghi thay đổi đó vào `references/tuy-chinh-rieng.md` (tạo file nếu chưa có, theo mẫu `references/tuy-chinh-rieng.md.example`) — TUYỆT ĐỐI không sửa trực tiếp vào `SKILL.md` hay các file `references/*.md` khác, vì các file đó do bản git dùng chung quản lý, máy không có quyền push sửa vào đó sẽ bị bản auto-update từ GitHub ghi đè mất ở lần cập nhật tiếp theo. Máy có quyền push GitHub (ADMIN/legion) muốn đổi quy chuẩn CHUNG cho cả team thì sửa thẳng file gốc + push như bình thường, không cần qua file tùy chỉnh riêng này.
